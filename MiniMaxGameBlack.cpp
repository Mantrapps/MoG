...
//MiniMax
// false = black move
computing MiniMaxGameBlack(int depth,mboard board, bool type)
{
    //need change
    computing comput;
    
    if (depth==0){
        comput.setEstimate(comput.comput_static_midend_black(board));
    ...
    if (type) //white move - maximize(true)
    {
        ...
        for(int i=0;i<List.size();i++)
        {
            ...
            tempcomput =MiniMaxGameBlack(depth-1, List.at(i), false);
    ...
    else
    {
        ...
        for(int i=0;i<List.size();i++)
        {
            ...
            tempcomput=MiniMaxGameBlack(depth-1, List.at(i), true);
...
int main(int argc, const char * argv[]) {
    ...
    computing layout=MiniMaxGameBlack(depth, board, false);
