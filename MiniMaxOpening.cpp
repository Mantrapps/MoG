...
//MiniMax
computing MiniMaxOpening(int depth,mboard board, bool type)
{
    //need change
    computing comput;
    ...
    if (type) //white move - maximize(true)
    {
        ...
        for(int i=0;i<List.size();i++)
        {
            ...
            tempcomput =MiniMaxOpening(depth-1, List.at(i), false);
    ...
    else
    {
        ...
        for(int i=0;i<List.size();i++)
        {
            ...
            tempcomput=MiniMaxOpening(depth-1, List.at(i), true);
            if(comput.getEstimate_value()>tempcomput.getEstimate_value())
            {
                comput.setEstimate(tempcomput.getEstimate_value());
                comput.setBoard(List.at(i));
...
int main(int argc, const char * argv[]) {
    ...
    computing layout=MiniMaxOpening(depth, board, true);
