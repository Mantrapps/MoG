...
//MiniMax
computing MiniMaxOpeningImproved(int depth,mboard board, bool type)
{
    
    computing comput;
    
    if (depth==0){
        comput.setEstimate(comput.comput_static_opening_improved(board));
    ...
    if (type) //white move - maximize(true)
    {
        ...
        for(int i=0;i<List.size();i++)
        {
            ...
            tempcomput =MiniMaxOpeningImproved(depth-1, List.at(i), false);
    ...
    else
    {
        ...
        for(int i=0;i<List.size();i++)
        {
            ...
            tempcomput=MiniMaxOpeningImproved(depth-1, List.at(i), true);
...
int main(int argc, const char * argv[]) {
    ...
    //improved
    computing layout=MiniMaxOpeningImproved(depth, board, true);
